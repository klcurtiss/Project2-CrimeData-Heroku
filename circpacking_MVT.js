...
d3.csv("DataForCirc/cleaned_data.csv", function(data) {
 ...
 // Turn the data into numeric value
  ...
  var mousemove = function(d) {
    Tooltip
      .html('<u>' + d.state_abbr + '</u>' + "<br>" + d.motor_vehicle_theft + " cars stolen")
  ...
  var node = group    
    .append("circle")
      .attr("class", "node")
      .attr("r", function(d){ return sizecircle(d.motor_vehicle_theft)})
      .attr("cx", width / 2)
      .attr("cy", height / 2)
      .style("fill", function(d){ return color(d.motor_vehicle_theft)})
  ...
  var text = group.append("text")
     .text(function(d) {
      ...
      .attr("font-size",function(d){ return sizetext(d.motor_vehicle_theft)})
  ...
  var simulation = d3.forceSimulation()
      ...
      .force("collide", d3.forceCollide().strength(.4).radius(function(d){ return (sizecircle(d.motor_vehicle_theft)+3) }).iterations(1)) // Force that avoids circle overlapping
