...
d3.csv("DataForCirc/cleaned_data.csv", function(data) {
 ...
 // Turn the data into numeric value
  ...
  var node = group    
    .append("circle")
      .attr("class", "node")
      .attr("r", function(d){ return sizecircle(d.aggravated_assault)})
      .attr("cx", width / 2)
      .attr("cy", height / 2)
  ...
  var simulation = d3.forceSimulation()
      ...
      .force("collide", d3.forceCollide().strength(.4).radius(function(d){ return (sizecircle(d.aggravated_assault)+3) }).iterations(1)) // Force that avoids circle overlapping
