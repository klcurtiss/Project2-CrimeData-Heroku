...
d3.csv("DataForCirc/cleaned_data.csv", function(data) {
 ...
 // Turn the data into numeric value
  ...
  var mappeddata = data1.map(d => d.homicide)
  
  // Create a scale for the colors
  var color = d3.scaleSequential(d3.interpolateOrRd).domain([0,d3.max(mappeddata)])
  ...
  var mousemove = function(d) {
    Tooltip
      .html('<u>' + d.state_abbr + '</u>' + "<br>" + d.homicide + " homicides")
  ...
  var node = group    
    .append("circle")
      .attr("class", "node")
      .attr("r", function(d){ return sizecircle(d.homicide)})
      .attr("cx", width / 2)
      .attr("cy", height / 2)
      .style("fill", function(d){ return color(d.homicide)})
  ...
  var text = group.append("text")
     .text(function(d) {
      ...
      .attr("font-size",function(d){ return sizetext(d.homicide)})
  ...
  var simulation = d3.forceSimulation()
      ...
      .force("collide", d3.forceCollide().strength(.4).radius(function(d){ return (sizecircle(d.homicide)+3) }).iterations(1)) // Force that avoids circle overlapping
